...
<!-- Add Skill Form -->
<form method="POST" action="{{ route('talent.profile.build.save', ['step' => 3]) }}">
    @csrf

    <h3 style="font-size: 18px; margin-bottom: 20px;">{{ $skills->count() > 0 ? 'Add Another Skill' : 'Add Skills' }}
    </h3>
    ...
    <div style="margin-bottom: 30px;">
        ...
        <select name="proficiency_level" id="proficiency_level" required
            ...
            <option value="intermediate" {{ old('proficiency_level')=='intermediate' ? 'selected' : '' }}>Intermediate
            </option>
    ...
    <div style="display: flex; justify-content: space-between; gap: 20px;">
        <a href="{{ route('talent.profile.build.step', ['step' => 2]) }}" class="primary-btn1 btn-hover"
        ...
        <div style="display: flex; gap: 10px;">
            ...
            <a href="{{ route('talent.profile.build.step', ['step' => 4]) }}" class="primary-btn1 btn-hover"

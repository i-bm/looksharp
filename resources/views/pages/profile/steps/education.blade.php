...
<form method="POST" action="{{ route('talent.profile.build.save', ['step' => 2]) }}">
    ...
    <h3 style="font-size: 18px; margin-bottom: 20px;">{{ $education->count() > 0 ? 'Add Another Education' : 'Add
        Education' }}</h3>
    ...
    <div style="margin-bottom: 20px;">
        ...
        <select name="degree_type" id="degree_type" required
            style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 16px;">
    ...
    <div style="margin-bottom: 20px;">
        ...
        <input type="number" name="gpa" id="gpa" value="{{ old('gpa') }}" step="0.01" min="0" max="5"
            placeholder="e.g., 3.5"
            style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 16px;">
        @error('gpa')
        <span style="color: red; font-size: 14px;">{{ $message }}</span>
    ...
    <div style="display: flex; justify-content: space-between; gap: 20px;">
        <a href="{{ route('talent.profile.build.step', ['step' => 1]) }}" class="primary-btn1 btn-hover"

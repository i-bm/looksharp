...
(function () {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const countdownSeconds = parseInt(
            resendSection.getAttribute("data-countdown-seconds") || "60",
            10
        );
        const otpSentAt = resendSection.getAttribute("data-otp-sent-at");
        const throttleRemaining = parseInt(
            resendSection.getAttribute("data-throttle-remaining") || "0",
            10
        );
        ...
        function handleResendClick() {
            ...
            if (csrfToken) {
                ...
                if (existingForm) {
                    const existingToken = existingForm.querySelector(
                        'input[name="_token"]'
                    );
                    if (existingToken) {
                        const csrfInput = document.createElement("input");
